...
betaParam <- function(deltaj,tau2,epsilon,ysquig,x){
  if(deltaj == 0){
    num <- sum(ysquig*x)
    denom <- sum(x^2) + 300/epsilon
    return(c(num/denom,1/denom))
  }else{
    num <- sum(ysquig*x)
    denom <- sum(x^2) + 300/tau2
    return(c(num/denom,1/denom))
  }
}
...
meanval <- matrix(nrow = 10,ncol = 20)
sigmaval <- matrix(nrow = 10,ncol = 20)
for(i in 2:20){
  for(j in 1:p){
    yCurr <- ysquiggle(y,X,betaSim[,i-1],j)
    deltaSim[j,i] <- rbernoulli(1,p = deltaParam(piSim[j,i-1],tau2,betaSim[j,i-1],epsilon))
    bParams <- betaParam(deltaTest[j],tau2,epsilon,yCurr,X[,j])
